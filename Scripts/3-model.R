...

{
  ...
  cat("
      ...
      logit(phi[t,i,j,k]) <- mu_phi + gamma_phi[j] + eta_phi[k] + pi_phi * SIC[j,k] + rho_phi * KRILL[j,k] #+ beta_phi*x[t]
...
jagsRun(jagsData = DATA, 
               ...
               jagsInits = F_Inits,
               params = Pars,
               jagsID = 'May_16_2018_gamma_eta_rho_pi_nu_3',
               jagsDsc = 'Four sites. Fewer iter than May 14 or May 16_1. logit(phi) <- mu + gamma + eta + rho + pi; logit(p) <- mu + beta*x + nu; No partial pooling. Long queue.',
