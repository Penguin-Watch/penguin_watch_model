...

{
  ...
  cat("
      ...
      logit(p[t,i,j,k]) <- mu_p + nu_p[i,j,k] + beta_p[j,k]*x[t] + eps[t,j,k]
...
jagsRun(jagsData = DATA, 
        ...
        jagsInits = F_Inits,
        params = Pars,
        jagsID = 'PW_60k_2019-04-12_FULL_beta[j,k]_eps[t,j,k]',
        jagsDsc = 'all sites/years (no missing)
        track z_out
        track p_out
        logit(phi) <- mu_phi[j,k];
        mu_phi_j_k ~ normal()
        logit(p) <- mu_p + nu_p[i,j,k] + beta_p[j,k] + eps[t,j,k]',
