...
# create AKER/CCAMLR df ---------------------------------------------------

#load krill data - determine which entries lie within sites we have data for

#AKER

# setwd(paste0(dir, 'Data/Krill_data/Processed_Aker/'))
# 
...
# #points are in 4326 (uses lat/lon)
# krill_points <- sp::SpatialPoints(krill_df, proj4string = CRS("+init=epsg:4326"))
# #transform to 3031 (to match everything else)
# t_krill_points <- sp::spTransform(krill_points, CRS(proj4string(Ant)))
# 
# 
...
#   k_temp <- dplyr::mutate(aker_krill_data[tind,], CCAMLR_Region = paste0(LABEL))
#   k_ll <- data.frame(LON = k_temp$lon_st,
#                      LAT = k_temp$lat_st)
#   if (NROW(k_ll) > 0)
#   {
#     k_pre <- sp::SpatialPoints(k_ll, proj4string = CRS("+init=epsg:4326"))
#     k_pts <- sp::spTransform(k_pre, CRS(proj4string(Ant)))
#   } else {
#     k_pre <- NA
#     k_pts <- NA
#   }
...
# {
#   #i <- 1
#   assign(paste0('aker_', SSMU_names[i], '_pts'), 
#          k_sp_fun(get(SSMU_names[i]), LABEL = SSMU_names[i], TYPE = 'points'))
#   assign(paste0('aker_', SSMU_names[i], '_krill'), 
#          k_sp_fun(get(SSMU_names[i]), LABEL = SSMU_names[i], TYPE = 'data'))
# }
...
#   
#   #filter by SSMU code
#   CC_kr_f <- dplyr::filter(CCAMLR_krill, SSMU_Code == SSMU_names[i])
#   
#   for (j in 1:length(yrs))
#   {
#     #j <- 1
#     
#     #CCAMLR krill catch for year
#     dec_d <- dplyr::filter(CC_kr_f, Calendar_Year == yrs[j], Month == 12)$Krill_Green_Weight
#     jan_nov_d <- dplyr::filter(CC_kr_f, Calendar_Year == yrs[j]+1, Month < 12)$Krill_Green_Weight
...
#     if (length(dates) > 0)
#     {
#       #total AKER krill caught over this period - IN TONNES (AKER IN KG)
#       dd <- temp_data[dates,]
...
# AP_SSMU_krill <- dplyr::filter(SSMU_out, ZONE %in% AP_SSMU_zones)
...
#   temp_trawls_150 <- dplyr::mutate(aker_krill_data[temp_in_buff_150,], col_id = cam_sites[i])
...
#   temp_trawls_100 <- dplyr::mutate(aker_krill_data[temp_in_buff_100,], col_id = cam_sites[i])
...
#   temp_trawls_50 <- dplyr::mutate(aker_krill_data[temp_in_buff_50,], col_id = cam_sites[i])
...
#   temp_trawls_25 <- dplyr::mutate(aker_krill_data[temp_in_buff_25,], col_id = cam_sites[i])
